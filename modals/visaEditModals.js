...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (type === "exp") {
      ...
      return interaction.reply({ content: "✅ Expiration mise à jour.", flags: 64 });
    }
    ...
    if (type === "emploi") {
      ...
      const fresh = await getVisaById(interaction.guildId, visaId);
      await refreshVisaMessage(interaction, fresh);
      return interaction.reply({ content: "✅ Emploi mis à jour.", flags: 64 });
    }
    ...
    if (type === "permis") {
      ...
      const fresh = await getVisaById(interaction.guildId, visaId);
      await refreshVisaMessage(interaction, fresh);
      return interaction.reply({ content: "✅ Permis mis à jour.", flags: 64 });
    }
    ...
    if (type === "raison") {
      ...
      const fresh = await getVisaById(interaction.guildId, visaId);
      await refreshVisaMessage(interaction, fresh);
      return interaction.reply({ content: "✅ Raison mise à jour.", flags: 64 });
    }
