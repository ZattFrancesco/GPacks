...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (action === "add" && step === "step1") {
      ...
      return interaction.reply({ content: "✅ Identité enregistrée. Clique pour continuer.", components: [row], flags: 64 });
    }
    ...
    if (action === "add" && step === "step2") {
      ...
      if (!dtStr) {
        return interaction.reply({ content: "❌ Date/heure invalide. Exemple heure: 18:00", flags: 64 });
      }

      const draft = getDraft(guildId, userId);
      if (!draft || draft.mode !== "add") {
        return interaction.reply({ content: "❌ Brouillon introuvable. Recommence avec ➕ Ajouter.", flags: 64 });
      }
      ...
      if (rec && weekForActions) {
        ...
        if (dup) {
          return interaction.reply({ content: "❌ Doublon: même date/heure et même ID accusé déjà présent.", flags: 64 });
        }
      }
      ...
      return interaction.reply({ content: "✅ Entrée ajoutée + planning mis à jour.", flags: 64 });
    }

    if (action === "edit" && step === "step1") {
      const idJudge = Number(maybeId);
      if (!idJudge) return interaction.reply({ content: "❌ Entrée invalide.", flags: 64 });
      ...
      return interaction.reply({ content: "✅ Infos enregistrées. Clique pour continuer.", components: [row], flags: 64 });
    }
    ...
    if (action === "edit" && step === "step2") {
      ...
      if (!draft || draft.mode !== "edit" || Number(draft.id_judge) !== idJudge) {
        return interaction.reply({ content: "❌ Brouillon introuvable. Recommence avec ✏️ Modifier.", flags: 64 });
      }
      ...
      return interaction.reply({ content: "✅ Entrée modifiée + planning mis à jour.", flags: 64 });
    }

    return interaction.reply({ content: "❌ Action modal inconnue.", flags: 64 });
  },
};
